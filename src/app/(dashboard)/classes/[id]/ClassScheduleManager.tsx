...
type ModalType = 'reschedule' | 'location' | null
...
export function ClassScheduleManager({ classId, className, location, dayOfWeek, startTime, endTime }: Props) {
  ...
  async function openModal(date: string, type: ModalType) {
    setLoadingDate(date)
    setError('')
    try {
  ...
  return (
    <>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    onClick={() => openModal(date, 'reschedule')}
                  ...
                  <button
                    onClick={() => openModal(date, 'location')}
                    disabled={!!loadingDate}
      ...
      {/* Modals */}
      {modalType === 'reschedule' && activeSession && activeDate && (
        <RescheduleModal
      ...
      {modalType === 'location' && activeSession && activeDate && (
        <ChangeLocationModal
          sessionId={activeSession.id}
          className={className}
          dayOfWeek={dayOfWeek}
