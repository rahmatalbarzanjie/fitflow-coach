...
export default function EventSettingsPage() {
  ...
  const { register, handleSubmit, reset, formState: { isSubmitting, isDirty } } = useForm<EventFormData>({
  ...
  useEffect(() => {
    async function load() {
      ...
      reset({
        ...
        status:              data.status,
        pricing_mode:        data.pricing_mode ?? 'tiered',
        tier1_label:         data.tier1_label ?? 'Early Bird',
        tier1_price:         Number(data.early_bird_price),
        tier1_quota:         data.early_bird_quota ?? undefined,
        tier2_label:         data.tier2_label ?? 'Regular',
        tier2_price:         Number(data.ots_price),
        tier2_quota:         data.max_capacity ?? undefined,
        ots_price:           Number(data.ots_price),
  ...
  return (
    <div className="w-full max-w-lg mx-auto pb-10">
      ...
      {/* STATUS — quick save, paling sering diubah */}
      <SectionList
        label="Status Event"
        footer="Ubah status untuk mempublikasikan atau menutup pendaftaran."
      >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <SectionList label="Informasi Event">
          <div className="px-4 py-4 space-y-4">
            ...
            <div>
              <label className={lbl}>
                Flyer / Cover <span className="text-gray-400 font-normal text-xs">(tampil di landing page)</span>
              </label>
              {coverUrl ? (
      ...
      <SectionList label="Zona Berbahaya" className="mt-4">
        <div className="px-4 py-4">
          <p className="text-xs text-gray-400 mb-4">
            Menghapus event akan menghapus semua data pendaftaran secara permanen.
          </p>
          {!confirmDel ? (
            <button
