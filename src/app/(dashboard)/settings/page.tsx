...
export default async function SettingsPage({
  ...
  const { welcome, tab = 'profil' } = await searchParams
  ...
  const pendingTestimonialCount = testimonials.filter((t: any) => !t.is_published).length
    ...
    <div className="max-w-lg">
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
        {TABS.map(t => (
          ...
          >
            {t.label}
            {t.key === 'testimoni' && pendingTestimonialCount > 0 && (
              <span className="ml-1.5 bg-orange-500 text-white text-xs px-1.5 rounded-full">{pendingTestimonialCount}</span>
            )}
          </Link>
        ))}
      </div>
