...
export function ClassGalleryUpload({ classId, userId, initialPhotos }: Props) {
  ...
  async function handleFile(file: File) {
    ...
    const { data: urlData } = supabase.storage.from('class-gallery').getPublicUrl(data.path)
    ...
    const { data: row, error: dbErr } = await supabase
      .from('class_gallery')
      .insert({ user_id: userId, class_id: classId, image_url: publicUrl, sort_order: nextOrder })
  ...
  async function removePhoto(id: string) {
    ...
    await supabase.from('class_gallery').delete().eq('id', id)
  }

  // Reorder ringan pakai tombol naik/turun (bukan drag-and-drop) - cukup
  // untuk galeri yang isinya puluhan foto, tanpa perlu library drag baru.
  async function move(id: string, dir: -1 | 1) {
    ...
    await Promise.all(next.map((p, i) => supabase.from('class_gallery').update({ sort_order: i }).eq('id', p.id)))
  }
      ...
      <div className="flex flex-wrap gap-3">
        {photos.map((p, i) => (
          <div key={p.id} className="relative w-20 h-20 rounded-xl overflow-hidden ring-2 ring-gray-200 ring-offset-2 group shrink-0">
            ...
            <img src={p.image_url} alt="dokumentasi kelas" className="w-full h-full object-cover" />
            <button
              type="button"
