...
export function FeedbackRequestButtonEvent({ eventId }: Props) {
  ...
  async function send() {
    ...
    setError('')
    try {
      const res = await fetch('/api/feedback/request-event', {
        method:  'POST',
  ...
  if (state === 'confirming') {
    return (
      <div className="flex flex-col items-end gap-2">
        <p className="text-xs text-gray-600">Kirim ucapan terima kasih + ajakan kritik & saran ke peserta yang hadir?</p>
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button
        ...
        className="flex items-center gap-1.5 h-9 px-4 border border-violet-200 text-violet-600 hover:bg-violet-50 rounded-xl text-sm font-medium disabled:opacity-50 transition-colors"
      >
        {state === 'sending' ? <Loader2 className="w-4 h-4 animate-spin" /> : <MessageSquareWarning className="w-4 h-4" />}
        Minta Kritik & Saran
      </button>
      {state === 'error' && (
