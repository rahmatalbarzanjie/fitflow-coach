...
import { Check, Loader2, RefreshCw, ChevronDown } from 'lucide-react'
...
export function ClassTypeWaGroupForm({
  ...
  // Nama grup yang terpilih (untuk tampilan)
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 space-y-3">
      ...
      {/* Link Invite */}
      <div>
        <p className="text-xs text-gray-500 mb-1">Link Invite Grup WA</p>
        <input
          type="text"
          value={link}
      ...
      {/* Grup WA — dropdown dari Fonnte */}
      <div>
        <div className="flex items-center justify-between mb-1">
          <p className="text-xs text-gray-500">Grup WA Komunitas</p>
          <button
            onClick={fetchGroups}
            disabled={loadingGrp}
        ...
        {grpError ? (
          ...
          <div className="h-9 flex items-center px-3 rounded-lg border border-gray-200 bg-gray-50">
            <span className="text-xs text-gray-400">Tidak ada grup WA ditemukan</span>
          </div>
        ) : (
        ...
        {/* Tampilkan nama grup yang tersimpan kalau belum load */}
        {groupId && !selectedGroup && !loadingGrp && (
          <p className="text-xs text-gray-400 mt-1">
            Grup tersimpan: <span className="font-mono">{groupId}</span>
          </p>
        )}
        {selectedGroup && (
          <p className="text-xs text-green-600 mt-1">
            ✓ Peserta yang chat di grup ini akan otomatis masuk komunitas {label}.
          </p>
        )}
      </div>
